...
function obtenerAprobados() {
  return JSON.parse(localStorage.getItem('cursosAprobados') || '[]');
}

function guardarAprobado(codigo) {
...
function requisitosCumplidos(curso, aprobados) {
  if (!curso.requisitos || curso.requisitos === '-' || curso.requisitos === 'Admisión') return true;
  return curso.requisitos.split(',').map(r => r.trim()).every(r => aprobados.includes(r));
}
...
function actualizarColores() {
  ...
  document.querySelectorAll('.celda').forEach(celda => {
    ...
    celda.classList.remove('completada', 'disponible');

    if (aprobados.includes(codigo)) {
      celda.classList.add('completada');
    } else if (requisitosCumplidos(curso, aprobados)) {
...
for (let semestre = 1; semestre <= 12; semestre++) {
  ...
  cursosSem.forEach(curso => {
    ...
    celda.dataset.codigo = curso.codigo;
